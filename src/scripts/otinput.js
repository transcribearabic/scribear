...
(function () {
  ...
  oTinput.prototype.isFormatSupported = function (format) {
    ...
    return !!(
      a.canPlayType && a.canPlayType('audio/' + format + ';').replace(/no/, '')
    );
  };
  ...
  oTinput.prototype._setupElement = function (element) {
    ...
    var altButton =
      '<button class="alt-input-button">' + altButtonText + '</button>';
    var urlInputText =
      this._text.altInputText ||
      'Enter URL of audio or video file, or YouTube video:';
    var urlInputClose = this._text.closeAlt || 'close';
    var urlInput =
      '<div class="ext-input-field" style="display: none;"><div class="close-ext-input">' +
      urlInputClose +
      '</div><label>' +
      urlInputText +
      '<input type="text"></label><div class="ext-input-warning"></div></div>';
  ...
  oTinput.prototype._setupMouseEvents = function () {
    ...
    $(element)
      .find('.alt-input-button')
      .click(function () {
        that.showURLInput();
      });
    $(element)
      .find('.close-ext-input')
      .click(function () {
        that.showFileInput();
      });
  };
  oTinput.prototype._reactToFile = function (input) {
    ...
    if (this.isFormatSupported(file)) {
      ...
      var err = new Error(
        'Filetype ' + file.type + ' not supported by this browser'
      );
  ...
  oTinput.prototype._reactToURL = function (url) {
    ...
    if (this.isFormatSupported(format)) {
      ...
      var err = new Error(
        'Filetype ' + format + ' not supported by this browser'
      );
